...
namespace WinFormApp_ProdInOut
{
    public partial class ProdInOut : Form
    {
        ...
        private void btnInpDetailQuery_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection mySqlConn = new SqlConnection(strConnString))
                {
                    ...
                    // 重點：使用欄位別名（AS）讓查詢結果更容易閱讀
                    // 重點：使用 WHERE 子句篩選日期範圍，起日後面加上 '999999' 以包含當天的所有時間
                    string strSQL = @"SELECT IH.chInpDateTime AS '進貨日期', IH.chFactNo AS '廠商代號',
                            F.chFactName AS '廠商名稱',ID.chProdNo AS '商品代號', P.chProdName AS '商品名稱',
                            ID.rlInpPrice AS '單價', ID.rlInpQty AS '數量', ID.rlInpAmt AS '單項金額'
                            from InputHeadTbl IH (nolock) 
                            left join InputDetailTbl ID (nolock) on IH.chInpDateTime = ID.chInpDateTime AND IH.chFactNo = ID.chFactNo
                            left join FactoryTbl F (nolock) on IH.chFactNo = F.chFactNo
                            left join ProductTbl P (nolock) ON ID.chProdNo = P.chProdNo
                            Where IH.chInpDateTime >= '" + txtInpDetailSDate.Text.Trim() + "' AND IH.chInpDateTime <= '" + txtInpDetailEDate.Text.Trim() + "999999'";

                    using (SqlCommand sqlCmd = new SqlCommand(strSQL, mySqlConn))
                    {
                        // 步驟四：執行查詢並載入 DataTable
                        DataTable dt = new DataTable();
                        dt.Load(sqlCmd.ExecuteReader());
                        ...
                        // 步驟六：將 DataTable 綁定到 DataGridView
                        // 重點：DataGridView 會自動根據 DataTable 的欄位產生對應的欄位
                        // 重點：SQL 查詢中的欄位別名會自動成為 DataGridView 的欄位標題
                        dgvInpDetailReport.DataSource = dt;

                        // 步驟七：計算總金額
                        // 請同學實作：計算總金額
                        // 重點：TryParse 會回傳 bool 值表示轉換是否成功，成功時將結果存入 out 參數
                        decimal totAmt = 0;
        ...
        // 進貨月報查詢
        private void btnInpMonthQuery_Click(object sender, EventArgs e)
        {
            // 步驟一：驗證日期格式
            if (txtInpMonthSDate.Text.Length != 5 || txtInpMonthEDate.Text.Length != 5)
            {
                MessageBox.Show("起迄日期需為5碼日期, eg.11412" + "\r\n\r\n" + "按任一鍵離開!", "日期錯誤!!!");
                txtInpMonthSDate.Focus();
                return;
            }

            // 步驟二：設定游標為等待狀態，並初始化總金額標籤
            this.Cursor = Cursors.WaitCursor;
            lblInpMonthTotAmt.Text = "";
            lblInpMonthTotAmt.Visible = false;
            try
            {
                using (SqlConnection mySqlConn = new SqlConnection(strConnString))
                {
                    ...
                    // 步驟三：撰寫 SQL 查詢語句（按日期和廠商統計）
                    // 使用 SUBSTRING 取得日期（前7碼：yyyMMdd），並按日期和廠商分組統計
                    string strSQL = @"SELECT SUBSTRING(IH.chInpDateTime, 1, 7) AS '日期',
                            F.chFactName AS '廠商名稱',
                            SUM(ID.rlInpAmt) AS '總金額'
                            from InputHeadTbl IH (nolock) 
                            left join InputDetailTbl ID (nolock) on IH.chInpDateTime = ID.chInpDateTime AND IH.chFactNo = ID.chFactNo
                            left join FactoryTbl F (nolock) on IH.chFactNo = F.chFactNo
                            Where IH.chInpDateTime >= '" + txtInpMonthSDate.Text.Trim() + "' AND IH.chInpDateTime <= '" + txtInpMonthEDate.Text.Trim() + "999999' " +
                    ...
                    using (SqlCommand sqlCmd = new SqlCommand(strSQL, mySqlConn))
                    {
                        ...
                        // 步驟六：將 DataTable 綁定到 DataGridView
                        dgvInpMonthReport.DataSource = dt;
                        ...
                        {
                            decimal amt;
                            if (decimal.TryParse(dt.Rows[i]["總金額"].ToString(), out amt))
                            {
                                totAmt += amt;
                            }
                        }
                        lblInpMonthTotAmt.Text = "總金額：" + Convert.ToString(totAmt);
                        lblInpMonthTotAmt.Visible = true;
                    }
                }
            }
            catch (Exception ex)
        ...
        // 進貨月報清除
        private void btnInpMonthClear_Click(object sender, EventArgs e)
        {
            lblInpMonthTotAmt.Text = "";
            lblInpMonthTotAmt.Visible = false;

            dgvInpMonthReport.DataSource = null;
        }
        ...
        // 出貨細項查詢
        private void btnOutDetailQuery_Click(object sender, EventArgs e)
        {
            // 步驟一：驗證日期格式
            if (txtOutDetailSDate.Text.Length != 7 || txtOutDetailEDate.Text.Length != 7)
            {
                MessageBox.Show("起迄日期需為7碼日期, eg.1141201" + "\r\n\r\n" + "按任一鍵離開!", "日期錯誤!!!");
                txtOutDetailSDate.Focus();
                return;
            }

            // 步驟二：設定游標為等待狀態，並初始化總金額標籤
            this.Cursor = Cursors.WaitCursor;
            lblOutDetailTotAmt.Text = "";
            lblOutDetailTotAmt.Visible = false;
            try
            {
                using (SqlConnection mySqlConn = new SqlConnection(strConnString))
                {
                    ...
                    using (SqlCommand sqlCmd = new SqlCommand(strSQL, mySqlConn))
                    {
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            MessageBox.Show("查詢區間內無出貨資料。" + "\r\n\r\n" + "按任一鍵離開!", "查無資料，敬請確認！！！");
                            this.Cursor = Cursors.Default;
                            return;
                        }

                        // 步驟六：將 DataTable 綁定到 DataGridView
                        dgvOutDetailReport.DataSource = dt;
                        ...
                        lblOutDetailTotAmt.Text = "總金額：" + Convert.ToString(totAmt);
                        lblOutDetailTotAmt.Visible = true;
                    }
                }
            }
            catch (Exception ex)
        ...
        // 出貨細項清除
        private void btnOutDetailClear_Click(object sender, EventArgs e)
        {
            lblOutDetailTotAmt.Text = "";
            lblOutDetailTotAmt.Visible = false;

            dgvOutDetailReport.DataSource = null;
        }

        // 出貨月報查詢
        private void btnOutMonthQuery_Click(object sender, EventArgs e)
        {
            // 步驟一：驗證日期格式
            if (txtOutMonthSDate.Text.Length != 5 || txtOutMonthEDate.Text.Length != 5)
            {
                MessageBox.Show("起迄日期需為5碼日期, eg.11412" + "\r\n\r\n" + "按任一鍵離開!", "日期錯誤!!!");
                txtOutMonthSDate.Focus();
                return;
            }

            // 步驟二：設定游標為等待狀態，並初始化總金額標籤
            this.Cursor = Cursors.WaitCursor;
            lblOutMonthTotAmt.Text = "";
            lblOutMonthTotAmt.Visible = false;
            try
            {
                using (SqlConnection mySqlConn = new SqlConnection(strConnString))
                {
                    ...
                    using (SqlCommand sqlCmd = new SqlCommand(strSQL, mySqlConn))
                    {
                        ...
                        // 步驟六：將 DataTable 綁定到 DataGridView
                        dgvOutMonthReport.DataSource = dt;
                        ...
                        lblOutMonthTotAmt.Text = "總金額：" + Convert.ToString(totAmt);
                        lblOutMonthTotAmt.Visible = true;
                    }
                }
            }
            catch (Exception ex)
        ...
        // 出貨月報清除
        private void btnOutMonthClear_Click(object sender, EventArgs e)
        {
            lblOutMonthTotAmt.Text = "";
            lblOutMonthTotAmt.Visible = false;

            dgvOutMonthReport.DataSource = null;
        }
    }
}
